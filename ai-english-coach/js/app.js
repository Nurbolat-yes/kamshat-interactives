...
const App = {
            ...
            <div class="ai-coach-container">
                ...
                ${!AI.isConfigured() ? `
                    <div style="padding: 12px 16px; background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.3); border-radius: var(--radius-md); margin-bottom: 16px; font-size: 0.85rem; color: #92400e;">
                        <strong>Note:</strong> AI is using built-in feedback. Configure a secure endpoint in Settings for full AI capabilities.
                    </div>
                ` : ''}
    ...
    showSettings() {
        ...
        if (endpointInput) endpointInput.value = settings.aiEndpoint || '';
    ...
    saveAISettings() {
        ...
        let finalEndpoint = customEndpoint;
        if (!finalEndpoint && apiKey) {
            finalEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=${encodeURIComponent(apiKey)}`;
        }

        Storage.saveSettings({
            aiConfigured: !!(apiKey || finalEndpoint),
            aiEndpoint: finalEndpoint || null,
            apiKey: apiKey || null
        });
